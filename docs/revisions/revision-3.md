...
*Working paper. Draft of September 2026. Revision 3.*
...
Regime-conditional return statistics use Newey–West (1987) standard errors, since sampling twelve-month forward returns monthly induces eleven months of overlap and inflates naive t-statistics by roughly √12. Against NBER dates the business-cycle model achieves recall of 0.906 but precision of only 0.326, producing 159 false-positive months across 27 episodes. The real-time Sahm rule — a single line of arithmetic — attains a higher F1 (0.525 vs 0.480).
...
Annualised quantities convert as *S*ₐₙₙ = *S*·√12 and δₐₙₙ = δ·√12. The variance expression is in **per-period** units and must be scaled by √12 before comparison with an annualised standard error; conflating the two understates the standard error by a factor of 3.46, an error we made in an earlier draft.
...
**Median ratio 1.25** (range 1.15–1.34). This is the genuine variance inflation from non-normality — the realised return series carries excess kurtosis of +1.59 and skew of −0.28 — and is the phenomenon Lo (2002) describes. Where a single figure is needed we use 1.25.
...
At *S*₂ = 0.682, 80% power, α = 0.05, including the measured 1.25× inflation:

| ρ | Δ = 0.05 | **Δ = 0.10** | Δ = 0.20 | Δ = 0.30 | Δ = 0.50 |
|---|---|---|---|---|---|
| 0.76 | 1950 | **489** | 123 | 55 | 20 |
| 0.80 | 1626 | **408** | 103 | 46 | 17 |
| 0.85 | 1221 | **306** | 77 | 35 | 13 |
| 0.89 | 896 | **225** | 57 | 26 | 10 |
| 0.92 | 652 | **164** | 41 | 19 | 7 |
| 0.95 | 408 | **103** | 26 | 12 | 5 |

The ρ range shown is not hypothetical: it spans the five comparisons actually made in Section 4.1. Detecting Δ = 0.10 requires between one and five centuries depending on which pair is compared — and the most favourable case, our own HMM against 60/40 at ρ = 0.952, still requires 103 years against the 44.6 available.
...
- **Δ ≥ 0.3** — requires 12–55 years depending on ρ. Testable within a career at high ρ; conventional inference applies and multiple-testing correction is the binding constraint.
- **Δ ≈ 0.2** — requires 26–123 years. Marginal at best, feasible only at high ρ with the longest available histories and no specification search.
- **Δ ≈ 0.1 or below** — requires 103–489 years. Not estimable from any sample that exists, at any ρ we observed.
...
**The closed form assumes i.i.d. returns.** We correct for the observed variance inflation empirically (1.25×, measured at pinned parameters in Section 7.2) but do not model the dependence structure analytically. Ledoit and Wolf (2008) provide the more rigorous route.
...
The explanation is not that regime models work, nor that they fail, but that a backtest of realistic length cannot resolve differences of the size at issue. Detecting a 0.1 Sharpe improvement requires between one and five centuries of monthly data, depending on a correlation parameter the literature rarely reports. Our study, at 44.6 years and by construction a careful example of its kind, had between a 5% and a 34% chance of detecting the effects it actually observed.
