...
*Working paper. Draft of September 2026. Revision 2.*
...
The explanation is statistical power, and we derive it in closed form. Using the Jobson–Korkie statistic with the Memmel correction, the sample length required to detect a true Sharpe difference Δ at 80% power is a simple function of Δ, the benchmark Sharpe, and the correlation ρ between the strategies. At our measured values (benchmark Sharpe 0.68, ρ = 0.89), detecting Δ = 0.10 requires **164 years** under Gaussian assumptions and approximately **224 years** once the observed variance inflation from fat tails is included. Monte Carlo simulation on bootstrapped real returns gives 213–231 years, agreeing with the analytic result. Our study attains **30% power** against its largest observed effect; its minimum detectable effect at 80% power is **0.20 Sharpe**.

The implication is bounded but consequential. Sharpe differences of 0.3 and above are detectable in 20–30 years and can be studied conventionally. Differences of 0.1 to 0.2 — the range in which tactical allocation overlays are evaluated, funded and marketed — require between six and twenty-five decades. When studies in that range are filtered for statistical significance before publication, the surviving literature is dominated by false positives. This applies with equal force to our own null results, which we accordingly decline to interpret as evidence that regime models do not work.
...
We are explicit that the third result constrains the first two. A study with 30% power cannot claim to have shown that look-ahead bias is unimportant.
...
Two conclusions changed materially. The price-momentum benchmark no longer outranks 60/40 (Section 4.3), which removes the support an earlier draft claimed for the "markets price regimes first" interpretation. And the specification-sensitivity result is smaller than first reported and is partly attributable to asset-set choice rather than sample window (Section 4.2). The power conclusion is unaffected.
...
We promote this from a secondary observation because it is the most immediately persuasive demonstration of the paper's thesis: a defensible specification choice moves the answer further than any bias we deliberately introduced.

| Sample start | Asset set | Years | Strategy | 60/40 | Δ | p |
|---|---|---|---|---|---|---|
...
The extremes differ by 0.166 Sharpe and carry opposite signs. A researcher reporting only the first row would describe a successful strategy; one reporting only the last would describe a failure. Neither is significant.
...
Against NBER dates the business-cycle model achieves recall of 0.906 but precision of only 0.326, producing 159 false-positive months across 27 episodes. The real-time Sahm rule — a single line of arithmetic — attains a higher F1 (0.525 vs 0.480).

Equity–bond correlation across regimes ranges only from 0.02 to 0.17 against an unconditional 0.08. Since regime-conditional allocation is substantially motivated by correlation instability, this weakens the mechanism in this sample.
...
All cells are evaluated on an identical 536-month sample with identical assets, weights, costs and trade lag. Sample alignment is essential: full-sample fitting emits a signal from the first observation while expanding-window cells begin only after burn-in, a gap spanning the entire Volcker disinflation.
...
Annualised quantities convert as *S*ₐₙₙ = *S*·√12 and δₐₙₙ = δ·√12; the variance expression is in per-period units and must be scaled accordingly before comparison with annualised standard errors.

Two features drive everything that follows. Required sample scales with **1/δ²**, so halving the effect quadruples the data needed. And the leading term is **2(1 − ρ)**: high correlation between strategy and benchmark sharply reduces the variance of the difference, which is why paired comparisons of similar strategies are more tractable than they first appear — and why ρ must be reported rather than assumed.

**Validation.** Against Monte Carlo simulation across five parameter combinations (Sharpe 0.5–1.0, ρ 0.85–0.98, T 300–1200), the closed form reproduces the simulated standard deviation to within 0.1–1.2%.

### 7.2 Application

At our measured values — benchmark Sharpe 0.682, ρ = 0.89, T = 535:

| Quantity | Value |
|---|---|
| Analytic (Memmel) SE of the Sharpe difference | 0.061 |
| Empirical bootstrap SE | 0.071 |
| Ratio (variance inflation from non-normality) | 1.17× |
| **MDE at 50% power** (= CI half-width) | **0.140** |
| **MDE at 80% power** | **0.200** |
| Power at largest observed effect (Δ = 0.102) | **30%** |

The analytic and empirical standard errors agree to within 17%, the excess reflecting the fat tails (excess kurtosis +1.59) and left skew (−0.28) of realised returns — the phenomenon Lo (2002) describes. We use the empirical value where a single figure is needed.

We flag a distinction an earlier draft got wrong. The half-width of a 95% confidence interval is the effect that would be *just* significant, corresponding to **50%** power — an even chance of detection. The conventional 80% threshold requires (z₀.₉₇₅ + z₀.₈₀) = 2.80 standard errors, not 1.96. Reporting a CI half-width as "the minimum detectable effect" understates the requirement by a factor of 1.43.

### 7.3 Years required

| True Δ Sharpe | Closed form (Gaussian) | With tail inflation | Monte Carlo |
|---|---|---|---|
| 0.05 | 653 | 894 | 858–930 |
| **0.10** | **164** | **224** | **213–231** |
| 0.20 | 41 | 57 | 55–58 |
| 0.30 | 19 | 26 | 25–27 |
| 0.50 | 7 | 10 | 9–10 |

Three independent methods agree. Monte Carlo figures carry simulation error of a few percent and are reproducible via `python -m macroregime.evaluation.power`.

Because required sample scales with 1/δ², the boundary is sharp. Δ = 0.30 is comfortably testable within a career. Δ = 0.20 requires roughly the full span of reliable monthly data. Δ = 0.10 requires more than two centuries.
...
- **Δ ≥ 0.3** — detectable in 20–30 years. Conventional inference applies, and multiple-testing correction is the binding constraint.
- **Δ ≈ 0.2** — requires 55–60 years. Marginal; feasible only with the longest available histories and no specification search.
- **Δ ≈ 0.1 or below** — requires two centuries or more. Not estimable from any sample that exists.
...
Considerable effort in this literature — including much of our own — targets look-ahead bias. Our ablation found the three principal channels jointly worth −0.001 Sharpe, with nothing surviving correction.

We are careful about what this licenses. It does **not** show look-ahead bias is unimportant; our power against effects of that size was under 15%. It shows the effort purchased no measurable change in conclusions here, and that a study confident it has eliminated look-ahead should not therefore be confident its results are informative.

The larger threats operate through **selection**. Our own specification-sensitivity result — a 0.166 swing that reverses sign, arising from the entirely defensible decision to include gold — exceeded every deliberately introduced bias combined.

### 8.4 Recommendations

**Report power alongside significance.** State the effect size the study could have detected. A null at 30% power is nearly uninformative, and reporting it as evidence of absence is an error.
...
**The closed form assumes i.i.d. returns.** We correct for the observed variance inflation empirically (1.17×) but do not model the dependence structure analytically. Ledoit and Wolf (2008) provide the more rigorous route.
...
*Citations require verification against original sources before submission; volume and page details have not been independently checked.*
...
| `power_closed_form.csv` | Section 7.3 sample-size requirements |
